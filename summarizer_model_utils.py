...
def sample_results(preds, ind2word, word2ind, converted_summaries, converted_texts, use_bleu=False):
    ...
    for pred, summary, text, seq_length in zip(preds[0],
        ...
        if beam:
            ...
            system_summari = ' '.join(actual_summary)
            reate_summari = ' '.join(created_summary)
            file = open('C:\\Users\\huynh\\Desktop\\tf\\text_summarization_vietnamese\\people-summary\\%d.txt'% i,'w+',encoding = 'utf-8')
            for item in system_summari:
            	item = re.sub(r'[_"“”\-;%()|+&=*%.,!?:#$@\[\]/]', ' ', item)
            	file.write(item)
            file2 = open('C:\\Users\\huynh\\Desktop\\tf\\text_summarization_vietnamese\\system_summary\\%d.txt' %i,'w+', encoding ='utf-8')
            for item2 in reate_summari:
            	item2 = re.sub(r'[_"“”\-;%()|+&=*%.,!?:#$@\[\]/]', ' ', item2)
            	file2.write(item2)
            if use_bleu:
