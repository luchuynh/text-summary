...
def clear_text(text,stops,remove_stopwords=True):
	# chu hoa thanh chu thuong
    text = text.lower()
    # dang dang va xoa cac u khong can thiet
...
def preprocess(text,stops,remove_stopwords=True):
    """
    ...
    """
    tokenized = []
    for ind, sentence in enumerate(nltk.sent_tokenize(text)):
        if ind <= 6:
            sentence = clear_text(sentence, stops)
...
def preprocess_texts_and_summaries(texts,summaries,stops,remove_stopwords=True):
    ...
    for text in texts:
        text = re.sub(r'[.]','. ',text)
        text = preprocess(text, stops,remove_stopwords=True)
        for word in text:
            words.append(word)
        processed_texts.append(text)
    for summary in summaries:
        summary = preprocess(summary, stops,remove_stopwords=True)
        for word in summary:
            words.append(word)
